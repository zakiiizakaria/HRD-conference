...
if (isset($_SERVER['HTTP_X_CLIENT_ERROR']) && $_SERVER['HTTP_X_CLIENT_ERROR'] === 'true') {
    try {
        ...
        logClientError('Client-side error during form submission', $errorData);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Check if the registration table exists, if not create it
        $pdo->exec("CREATE TABLE IF NOT EXISTS registration_submissions (
        ...
        $stmt = $pdo->prepare("INSERT INTO registration_submissions (full_name, email, company, job_title, contact_number, promo_code) VALUES (?, ?, ?, ?, ?, ?)");
